...
bool buttonA_pressed = false, buttonC_pressed = false;

void loop()
{
  M5.update();
  buttonA_pressed |= M5.BtnA.isPressed();
  buttonC_pressed |= M5.BtnC.isPressed();

  unsigned long now = millis();
  if (now - scratchlink_msec > 1000) {
    scratchlink_msec = now;
    scratchlink_update(0, 0, buttonA_pressed, buttonC_pressed, false, false, false, 0);
    buttonA_pressed = buttonC_pressed = false;
  }
  delay(10);
}
